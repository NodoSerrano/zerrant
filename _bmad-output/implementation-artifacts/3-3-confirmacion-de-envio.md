...
- T4: suite green after recovery onto current main.
- 2026-08-28: Recovered from closed PR #19 (`2ef4461`) after #24 merged ZER-27 only and left 1.9 out of main. Also kept action harden (tourist gate, already-pending, Spanish errors) + unique partial index migration. Fixed duplicate `roles`/`profile_roles` in `database.types.ts` (typecheck blocker on main).

### Change Log

- 2026-08-13: Implemented ZER-26 confirmation screen 1.9 (Nobel, PR #19 — closed without merge).
- 2026-08-28: Re-landed onto main post-#24; PR recovery of ZER-26 only.
...
- supabase/migrations/20260813210000_membership_requests_one_pending.sql
